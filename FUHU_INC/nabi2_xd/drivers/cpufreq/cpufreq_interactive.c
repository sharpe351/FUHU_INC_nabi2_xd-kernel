...
//&*&*&*HC1_20121102, add NV's patch to fix node permission issue
#define CONFIG_SWITCH 1

#ifdef CONFIG_SWITCH
#include <linux/switch.h>
#endif
//&*&*&*HC2_20121102, add NV's patch to fix node permission issue
...
static ssize_t switch_modeset_print_mode(struct switch_dev *sdev, char *buf)
{
	...
	else if (sdev->state==2)
		return sprintf(buf, "stop\n");
	else if (sdev->state==3)
		return sprintf(buf, "limit\n");
}

static struct switch_dev cpufreq_switch_dev = {
	.name = "cpufreq_interactive",
	.print_state = switch_modeset_print_mode,
};

static void cpufreq_switch_set_state(int state)
{
	switch_set_state(&cpufreq_switch_dev, state);
}
#endif
//&*&*&*HC2_20121102, add NV's patch to fix node permission issue
...
//&*&*&*HC1_20121102, add NV's patch to fix node permission issue
#ifdef CONFIG_SWITCH
	cpufreq_switch_set_state(event);
#endif
//&*&*&*HC2_20121102, add NV's patch to fix node permission issue
...
//&*&*&*HC1_20121102, add NV's patch to fix node permission issue
#ifdef CONFIG_SWITCH
	/* Addd cpufreq swith class support */
	ret = switch_dev_register(&cpufreq_switch_dev);
	if (ret < 0)
		switch_dev_unregister(&cpufreq_switch_dev);
#endif
//&*&*&*HC2_20121102, add NV's patch to fix node permission issue
