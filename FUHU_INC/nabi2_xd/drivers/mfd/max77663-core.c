...
/*
  * Subject: [PATCH 1/3] mfd: max77663: unmask ONOFF_IRQ_EN0_FALLING irq_mask
  */
...
#define MAX77663_REG_ONOFF_CTRL_STAT	0x15	//NJ5+
...
#define ONOFF_STAT_ACOK_MASK	(1 << 1) 	//NJ5+
...
//&*&*&*HC1_20130411, add NV's patch for bug 1267529
/* unmask ONOFF_IRQ_EN0_FALLING irq_mask */
#define ONOFF_IRQ_EN0_FALLING		(1 << 2)
//&*&*&*HC2_20130411, add NV's patch for bug 1267529
...
//&*&*&*HC1_20130220, disable rebooting feature while cable is attached
#if 1
...
#endif	
//&*&*&*HC2_20130220, disable rebooting feature while cable is attached	
...
{	
	...
	if (chip->enter_early_suspend == 1 && chip->wakeup_event == 0) {
		printk("[max77663-core]enter %s, send power key event.\n", __func__);
...
static int max77663_irq_init(struct max77663_chip *chip)
{
	...
	//&*&*&*HC1_20130411, add NV's patch for bug 1267529
	/* unmask ONOFF_IRQ_EN0_FALLING irq_mask */	
	chip->cache_irq_mask[CACHE_IRQ_ONOFF] &= ~ONOFF_IRQ_EN0_FALLING;
	//&*&*&*HC2_20130411, add NV's patch for bug 1267529
	max77663_write(chip->dev, MAX77663_REG_ONOFF_IRQ_MASK,
		       &chip->cache_irq_mask[CACHE_IRQ_ONOFF], 1, 0);
...
static void max77663_early_suspend(struct early_suspend *handler)
{
	...
	//&*&*&*AL1_20130109
	#ifdef CONFIG_BATTERY_BQ27x00
	register_low_battery_callback(max77663_low_battery_alert, chip);
	#endif
	//&*&*&*AL2_20130109
}
