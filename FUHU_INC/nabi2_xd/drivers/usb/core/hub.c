...
//&*&*&* Simon (1), Keep the hdev for EHCI used 06/08/2012
static struct usb_device *modem_usb_hdev; //Simon
//&*&*&* End: (1)
...
//&*&*&*SJ1_20120521, fix usb otg can't detect USB OTG devices.
	{
		...
		//&*&*&* Simon (2), Keep the hdev for EHCI used 06/08/2012
		if ('1' == usb_device_name[0]) { /* usb1 for CL2N modem  */
			dev_info (&intf->dev, "modem_usb_hdev is  usb%c \n", usb_device_name[0]);
			usb_disable_autosuspend(hdev);
			modem_usb_hdev = hdev ; //Simon
		}		
		//&*&*&* End (2)
	}
//&*&*&*SJ2_20120521
...
//&*&*&* Simon (3), Keep the hdev for EHCI used 06/08/2012
void FX_Wake_EHCI()
{
   printk( "[Simon]FX_Wake_EHCI \n");
	 usb_disable_autosuspend(modem_usb_hdev);	
}
EXPORT_SYMBOL_GPL(FX_Wake_EHCI);
//&*&*&* End: (3)