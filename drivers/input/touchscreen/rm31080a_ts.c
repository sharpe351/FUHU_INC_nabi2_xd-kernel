...
#define WDT_NORMAL_TIME		5//00 /* 1 sec */
...
static ssize_t rm_tch_version_show(struct device *dev,
        ...
        char *buf)
{
	return sprintf(buf, "Release V 0x%02X, Test V 0x%02X, Cal V 0x%02X\n", g_stTs.u8Version, g_stTs.u8TestVersion, g_stTs.u8CalVersion);
}
...
#else
static DEVICE_ATTR(slowscan_enable, 0644, rm_tch_slowscan_show,
        rm_tch_slowscan_store);
static DEVICE_ATTR(smooth_level, 0644, rm_tch_smooth_level_show,
        rm_tch_smooth_level_store);
static DEVICE_ATTR(self_test, 0644, rm_tch_self_test_show,
        rm_tch_self_test_store);
static DEVICE_ATTR(version, 0644, rm_tch_version_show,
        rm_tch_version_store);
static DEVICE_ATTR(module_detect, 0644, rm_tch_module_detect_show,
        rm_tch_module_detect_store);
#endif
...
dev_info(&g_spi->dev,"Raydium TS:Firmware v%d.%d, Cal V 0x%02X \n",
			        g_stTs.u8Version, g_stTs.u8TestVersion, g_stTs.u8CalVersion);
			break;
        case RM_VARIABLE_CAL_VERSION:         
            g_stTs.u8CalVersion = (u8) arg;
            break;    
...
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd & 0xFFFF) {
		...
		case RM_IOCTL_GET_PARAMETER:
			...
			rm_tch_set_input_resolution(g_stCtrl.u16ResolutionX, g_stCtrl.u16ResolutionY);
			break;
