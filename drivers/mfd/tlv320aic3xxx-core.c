...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_write_device(aic3xxx, page_buf[0], &page_buf[1], 1);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_write_device(aic3xxx, book_buf[0], &book_buf[1], 1);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_read_device(aic3xxx, offset, &val, 1);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_read_device(aic3xxx, offset, buf, count);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_write_device(aic3xxx, offset, &val, 1);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_write_device(aic3xxx, offset, buf, count);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_read_device(aic3xxx, offset, &r, 1);
#else	
...
#if (AIC3206_SPI_RW_REG)
	ret = aic3xxx_spi_write_device(aic3xxx, offset , &r, 1);
#else	
...
//&*&*&*CT1_130221:	unuse gpio pin to reset codec
	/*
...
//&*&*&*CT2_130221:	unuse gpio pin to reset codec

	/* run the codec through software reset */
/*	ret = regmap_write(aic3xxx->regmap, AIC3XXX_RESET, resetVal);	*/
...
/*	ret = regmap_read(aic3xxx->regmap, AIC3XXX_DEVICE_ID, &value);	*/
...
//&*&*&*CT1_130219: after adb reboot, device id is read as 16. So just read D0-D3 value
	ret = ret & 0xf;
//&*&*&*CT2_130219: after adb reboot, device id is read as 16. So just read D0-D3 value

	dev_info(aic3xxx->dev, "AIC3XXX_DEVICE_ID: %d\n", ret);
	...
	dev_info(aic3xxx->dev, "TLV320AIC3256\n");
		break;
	case 3:
		...
		aic3xxx->type = TLV320AIC3262;
			dev_info(aic3xxx->dev, "TLV320AIC3262\n");
		break;
	...
	case TLV320AIC3266:
		break;
	case TLV320AIC3256:
		dev_info(aic3xxx->dev, "calling mfd_add_devices for 3256\n");
		ret = mfd_add_devices(aic3xxx->dev, -1,
...
err_mfd:
	dev_info(aic3xxx->dev, "aic3xxx_irq_exit\n");
